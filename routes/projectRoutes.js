...
function generateSignedUrl({ filename, username, task, project, version, userId }, expiresInSeconds = 300) {
  const token = jwt.sign(
    { filename, username, task, project, version, userId},
    D_S_K,
...
router.get('/get-download-url/:filename', (req, res) => {
  ...
  const { username, task, project, version } = req.query; // Optional, if needed to validate outside token
  ...
  const signedUrl = generateSignedUrl({ filename, username, task, project, version, userId }, 600);
